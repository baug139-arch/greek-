...
const tWordsAttr = `words={useMemo(() => {
...
if (code.includes(tWordsAttr)) {
  code = code.replace(tWordsAttr, `words={activeMorphologyWords}`);
  
  const insertionPoint = `  return (
    <div className="flex h-screen w-full bg-[#FDFCFB] text-[#1A1A1A] font-serif overflow-hidden">`;
    
  const newMemoCode = `  const activeMorphologyWords = React.useMemo(() => {
  ...
  code = code.replace(insertionPoint, newMemoCode);
  
  fs.writeFileSync('src/App.tsx', code);
  console.log('Patched App.tsx with activeMorphologyWords');
} else {
  console.log('Failed to patch App.tsx, target words attr not found');
}
